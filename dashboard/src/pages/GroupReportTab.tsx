...
function criterionColor(name: string): string {
  if (name.includes("Specialist")) return "text-violet-400";
  if (name.includes("Decision")) return "text-amber-400";
  if (name.includes("Communication")) return "text-cyan-400";
  return "text-zinc-400";
}

function criterionBorder(name: string): string {
  if (name.includes("Specialist")) return "border-violet-500/30";
  if (name.includes("Decision")) return "border-amber-500/30";
  if (name.includes("Communication")) return "border-cyan-500/30";
  return "border-zinc-700/30";
}

export default function GroupReportTab() {
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6 min-w-0">
      ...
      <div className="rounded-xl border border-zinc-700/40 bg-zinc-900/60 p-5">
        ...
        <div className="mt-2 text-[9px] text-zinc-500">
          AI Policy: {REPORT_META.aiPolicy} &middot; Submission: {REPORT_META.submission}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-zinc-700/40 bg-zinc-900/60 p-4">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2">
          ...
          <div className="text-[9px] text-zinc-600 ml-auto">
            + Cover, Exec Summary, Intro, References, Appendices (not counted)
          </div>
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-baseline justify-between">
                  ...
                  <span className={`text-[18px] font-black ${criterionColor(crit.name)} opacity-70`}>
                    {crit.weight}
                  </span>
                </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {sec.countsTowardLimit ? (
                    <span className="text-[7px] font-bold tracking-wider text-amber-500/60 bg-amber-900/20 px-1.5 py-0.5 rounded">
                      COUNTS TOWARD LIMIT
                    </span>
                  ) : (
                    <span className="text-[7px] font-bold tracking-wider text-zinc-600 bg-zinc-800/40 px-1.5 py-0.5 rounded">
                      NOT COUNTED
                    </span>
                  )}
