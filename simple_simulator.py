...
class MissionSimulator:
    def __init__(self, config_file='mission_config.json'):
        ...
        # NFZ buffer: auto-pause when drone gets within this distance of NFZ edge
        self.nfz_buffer = 7.5
        self.nfz_warning_active = False  # prevent repeated triggers
        ...
        self.fig.subplots_adjust(right=0.78)  # Make room for info panel on the right
        self.setup_plot()
    ...
    def setup_plot(self):
        ...
        if len(self.no_fly_zone) > 2:
            ...
            # NFZ buffer zone - dashed outline offset outward by nfz_buffer
            nfz = self.no_fly_zone
            n = len(nfz)
            # Determine winding order via shoelace signed area
            signed_area = sum(
                nfz[i][0] * nfz[(i+1)%n][1] - nfz[(i+1)%n][0] * nfz[i][1]
                for i in range(n)
            )
            # Flip sign so normals point outward from polygon
            ...
            for i in range(n):
                ...
                x1, y1 = nfz[i]
                x2, y2 = nfz[(i + 1) % n]
                # Average outward normal at vertex (flip based on winding)
                nx1, ny1 = -(y1 - y0) * sign, (x1 - x0) * sign
                nx2, ny2 = -(y2 - y1) * sign, (x2 - x1) * sign
                l1 = math.sqrt(nx1**2 + ny1**2) or 1
                l2 = math.sqrt(nx2**2 + ny2**2) or 1
                nx = nx1/l1 + nx2/l2
                ny = ny1/l1 + ny2/l2
                ln = math.sqrt(nx**2 + ny**2) or 1
                buffer_pts.append((x1 + self.nfz_buffer * nx/ln, y1 + self.nfz_buffer * ny/ln))
            buf_poly = patches.Polygon(buffer_pts, closed=True,
                                       edgecolor='#e03131', facecolor='none',
                                       alpha=0.5, linewidth=1.5, linestyle='--',
                                       label='NFZ Buffer')
            self.ax.add_patch(buf_poly)
        ...
        # Info text - positioned outside plot area on the right
        self.info_text = self.fig.text(0.80, 0.95, '', verticalalignment='top',
                                      fontsize=10, family='monospace', color='#e0e0e0',
    ...
    def distance_to_nfz(self):
        """Calculate minimum distance from drone to nearest NFZ polygon edge"""
        if len(self.no_fly_zone) < 3:
            return float('inf')

        px, py = self.drone.x, self.drone.y
        min_dist = float('inf')
        n = len(self.no_fly_zone)

        for i in range(n):
            x1, y1 = self.no_fly_zone[i]
            x2, y2 = self.no_fly_zone[(i + 1) % n]
    ...
    def check_no_fly_violation(self):
        """Check NFZ: inside = violation logged, within buffer = auto-pause to manual"""
        if len(self.no_fly_zone) < 3:
            return False

        inside = self.point_in_polygon((self.drone.x, self.drone.y), self.no_fly_zone)
        ...
        if inside:
            ...
            if self.state != MissionState.MANUAL:
                ...
                self.nfz_warning_active = True
                self.ax.set_title('NFZ VIOLATION - Press A to resume, L to land',
                                 fontsize=14, fontweight='bold', color='#e03131')
                print("NFZ BREACH! Drone paused in MANUAL mode.")
            return True

        # Buffer check - only when not already in manual from NFZ warning
        dist = self.distance_to_nfz()
        if dist < self.nfz_buffer:
            if not self.nfz_warning_active and self.state != MissionState.MANUAL:
                self.nfz_warning_active = True
                self.pre_manual_state = self.state
                self.state = MissionState.MANUAL
                self.ax.set_title(f'NFZ WARNING ({dist:.0f}m away) - Press A to resume, L to land',
                                 fontsize=13, fontweight='bold', color='#ffa94d')
                print(f"\nNFZ BUFFER WARNING! Drone is {dist:.0f} units from no-fly zone.")
                print("Auto-paused to MANUAL mode. Press A to resume or L to emergency land.\n")
                plt.draw()
            return False
        else:
            # Clear warning once we're safely away
            self.nfz_warning_active = False
    ...
    def update_visualization(self):
        ...
        nfz_dist = self.distance_to_nfz()
        if nfz_dist < self.nfz_buffer * 2:
            info += f"\nNFZ Distance: {nfz_dist:.0f} units"
        info += f"\nViolations: {len(self.violations)}"
        if self.state == MissionState.MANUAL:
            if self.nfz_warning_active:
                info += "\n\n>>> NFZ WARNING <<<\n"
            else:
                info += "\n\n>>> MANUAL MODE <<<\n"
