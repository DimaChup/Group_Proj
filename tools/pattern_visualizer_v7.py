...
def generate_zian_spiral(search_poly_gps, strip_spacing_m, drone_gps=None,
    ...
    while k < max_laps:
        ...
        # Don't close the loop — leave it open so the path transitions
        # smoothly to the next inner ring (continuous spiral)
    ...
    def _generate_pattern(self):
        ...
        if self.pattern_type == "zian_spiral":
            ...
            try:
                ...
                _zian_dir = os.path.join(_proj_root, "Zian", "path_planner")
                def _load_zian(name):
                    spec = _ilu.spec_from_file_location(
                        name, os.path.join(_zian_dir, f"{name}.py"))
            ...
            while True:
                ...
                if inset.geom_type != 'Polygon':
                    break
                # Simplify to corners only
                simplified = inset.simplify(strip_spacing_m * 0.1, preserve_topology=True)
                if simplified.is_empty or simplified.geom_type != 'Polygon':
                    break
                verts = list(simplified.exterior.coords)[:-1]  # drop closing dup
                if len(verts) >= 3:
                    shells.append(verts)
                offset += strip_spacing_m

            # Build ONE continuous inward-spiraling line:
            # Walk each shell almost all the way around, but just before
            # returning to the start vertex, cut inward to the next shell.
            # Result: one line that spirals from outer edge to center.
            spiral_waypoints = []

            def _to_gps(v):
                return (v[1] / 111320 + ref_lat, v[0] / (111320 * cos_lat) + ref_lon)

            def _nearest_idx(verts, ref_pt):
            ...
            def _add(v):
                gps = _to_gps(v)
                if not spiral_waypoints or gps != spiral_waypoints[-1]:
                    spiral_waypoints.append(gps)

            # Determine entry point
            last_m = None
            if self.drone_gps:
                last_m = ((self.drone_gps[1] - ref_lon) * 111320 * cos_lat,
                          (self.drone_gps[0] - ref_lat) * 111320)

            for shell_idx, verts in enumerate(shells):
                n = len(verts)
                is_last = (shell_idx == len(shells) - 1)

                # Start from vertex nearest to where we left off
                start = _nearest_idx(verts, last_m) if last_m else 0

                if is_last:
                    # Last shell: walk fully closed, then go to center
                    for k in range(n + 1):
                        _add(verts[(start + k) % n])
                else:
                    # Walk all n vertices (full perimeter) but DON'T close.
                    # The last vertex we emit is one step before returning
                    # to start — from there we jump to the next inner shell.
                    for k in range(n):
                        _add(verts[(start + k) % n])
                    # last_m = the last vertex we emitted (for next shell connection)
                    last_m = verts[(start + n - 1) % n]

            # Final point: polygon centroid
            centroid = shape_poly.centroid
            if not centroid.is_empty:
                _add((centroid.x, centroid.y))
