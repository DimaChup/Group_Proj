...
def main():
    ...
    # Load models (TFLite + optional NCNN)
    models = {}
    model_names = {}
    ...
    print(f"Press M to switch models ({len(models)} available)\n")
    ...
    while True:
        ...
        if frame is not None:
            ...
            backend = vs.backend_name.upper() if hasattr(vs, 'backend_name') else "?"
            model_tag = model_names.get(active_model[0], f"Model {active_model[0]}") + f" [{backend}]"
            eff_name = EFFECTS[active_effect[0]].upper()
            eff_str = f" | WX: {eff_name} {effect_intensity[0]:.0%}" if eff_name != "NONE" else ""
            eff_fps = 1000.0 / dt if dt > 0 else 0
            info = f"{timestamp:.1f}s/{duration:.1f}s | Inf: {dt:.0f}ms ({eff_fps:.1f}fps) | Video: {fps*speed:.0f}fps | Det: {det_count[0]} | {mode} | {speed:.1f}x | [{model_tag}] M=switch{eff_str}"
