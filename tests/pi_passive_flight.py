...
def main():
    ...
    if write_header:
        writer.writerow([
            ...
            "dummy_expected_px", "battery_v", "battery_pct"
        ])
    ...
    try:
        while True:
            ...
            if found:
                ...
                event_log.append((
                    ts,
                    f"DETECT #{detect_count}  conf={conf:.2f}  alt={alt:.1f}m  {gps_short}",
                    (0, 255, 0)
                ))
            ...
            if HEADLESS:
                if found:
                    print(f"  ** DETECTED ** conf={conf:.2f} at ({px},{py})  "
                          f"alt={alt:.1f}m  GPS=({telem['lat']:.6f},{telem['lon']:.6f})")
