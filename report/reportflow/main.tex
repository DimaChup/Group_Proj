...
With the operational context established, we turn to a key architectural decision: the choice of inference backend for the Raspberry Pi~5. This decision directly constrains the detection and energy dimensions. Table~\ref{tab:mcda-backend} presents a multi-criteria decision analysis (MCDA) comparing four viable options, each scored 1--5 and weighted by importance to the mission.
...
\textbf{What we did.} We retrained the YOLOv8n model~\cite{yolov8} on a mixed dataset of 300~synthetic images (using domain randomisation techniques~\cite{synthdata}), 16~manually labelled real flight frames, and 50~hard-negative backgrounds at the camera's native 1456$\times$1088 resolution. Validation yielded mAP50\,=\,0.995.
...
Table~\ref{tab:sar-comparison} positions this project against published SAR drone systems. Direct comparison is imperfect---differing hardware, environments, and target types---but provides context that internal metrics alone cannot.
...
The 58 test scripts and 127 unit test functions are integral to the optimisation methodology, providing the empirical evidence that anchors every link in the decision chain.
