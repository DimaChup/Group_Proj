...
This report presents the design, optimisation, and verification of an autonomous Search and Rescue (SAR) drone system that locates a casualty within an irregular polygon bounded by a protected ecological no-fly zone (SSSI). Drawing on SAR robotics methodology~\cite{murphy2014} and coverage path planning theory~\cite{galceran2013survey}, the system comprises a Raspberry Pi~5 running a retrained YOLOv8n detector~\cite{yolov8} (mAP50\,=\,0.995, 4.8\,FPS TFLite / 9\,FPS NCNN), a 20-state finite state machine atop the ArduPilot autopilot~\cite{ardupilot}, and a lawnmower path planner with layered geofencing. A physics-based 216-configuration parameter sweep identified the optimal operating point (35\,m altitude, 8\,m/s, 70\textdegree{} scan angle), yielding $>$99.97\% single-pass detection probability at 10.9\% battery usage with a 1.5$\times$ NFZ safety margin. Video-replay field testing confirmed GPS target estimation accuracy of CEP50\,=\,2.3\,m. Twelve formal requirements were verified against quantitative evidence (10~met, 2~partially met), and a plus/delta evaluation identifies the remaining gaps before full autonomous flight.

% ════════════════════════════════════════════════════════════════════════
\section{Objective}\label{sec:df-objective}

The mission objective is to \textbf{locate a casualty as quickly as possible} within a defined search area, using autonomous aerial search with onboard computer vision.

% ════════════════════════════════════════════════════════════════════════
\section{Constraints}\label{sec:df-constraints}

This objective is bounded by hard constraints that cannot be relaxed and that shape every downstream design choice:
...
\textbf{The decision.} Lawnmower (boustrophedon) coverage is the standard method recommended by the International Aeronautical and Maritime Search and Rescue Manual (IAMSAR Vol.~III)~\cite{wca1981} and is widely deployed on autonomous marine and aerial SAR platforms~\cite{galceran2013survey,choset2001coverage}. It provided the best coverage guarantee for irregular polygons with NFZ constraints. The search area is a 5-corner irregular polygon with an SSSI no-fly zone on one edge --- not a convex shape where spiral patterns excel. The lawnmower pattern's deterministic lane structure means we can \emph{prove} that every accessible point is scanned, which is essential for a safety-critical SAR mission~\cite{murphy2014}. However, spiral inward patterns were also considered for their lower turn count. In a convex search area without NFZ constraints, a spiral would likely be the better choice.
...
Without correction, forward flight pitch introduces up to 6.2\,m offset ($h\tan\theta = 35\tan 10^{\circ}\approx 6.2$\,m)---larger than all other error sources combined.  Attitude-compensated estimation (\texttt{-{}-compensate-tilt}) reduces this to ${\sim}0.3$\,m by ray-tracing each detection through the full rotation matrix $\mathbf{R} = \mathbf{R}_z(\psi)\,\mathbf{R}_y(-\theta)\,\mathbf{R}_x(-\phi)$ constructed from the drone's \texttt{ATTITUDE} telemetry, which is the exact mathematical inverse of the camera model used in simulation.  This was verified with 1440 automated test cases across all altitudes and tilt angles, each producing errors below 0.01\,m.  The centering procedure further mitigates residual error by hovering ($\theta < 1^{\circ}$) before the final landing estimate.
...
The system scores 7.7/10 in simulation capability (all states exercised, energy model validated, detection pipeline proven) but only 2.1/10 in real-world readiness: bench and video-replay testing are complete, yet no autonomous outdoor flight has been conducted. The 2.1 reflects the absence of Stages~6--7 (manual passive flight and full autonomous mission) in the simulation-to-real progression. The gap between simulation confidence and field-proven reliability is the project's primary risk, driven by weather cancellations and scheduling constraints rather than technical blockers. The optimisation framework itself is re-evaluable: if field data shifts the detection envelope, the entire chain can be re-run in minutes to produce updated parameters.


% ════════════════════════════════════════════════════════════════════════
\section{Evidence-Based Adaptation}\label{sec:decisions-changed}

A rigorous engineering process is measured not by whether initial assumptions hold, but by how quickly errors are detected and corrected. Four significant corrections were made during development --- each discovered through systematic testing, each requiring a different diagnostic approach, and each validating the modular architecture that isolated the fix to a single module.
...
Four mid-development corrections --- BGR colour fix, focal length recalibration, TFLite coordinate bug, and calibration corruption --- demonstrated that the modular architecture contained errors to single files without ripple effects. The 58 test scripts and 127 unit tests provided the empirical foundation for every link in the decision chain, while the seven-stage simulation-to-real progression introduced each new variable incrementally. The framework can be re-evaluated in minutes as field data replaces simulation assumptions, closing the gap between demonstrated simulation capability (7.7/10) and the outdoor flight validation (2.1/10) that remains the next critical milestone.
