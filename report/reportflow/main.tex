...
This report presents the design, optimisation, and verification of an autonomous Search and Rescue (SAR) drone that locates a casualty within an irregular polygon bounded by a protected ecological no-fly zone (SSSI). following established SAR robotics methodology~\cite{murphy2014,lyu2023uav_sar_survey} and coverage path planning theory~\cite{galceran2013survey}, the system combines a Raspberry Pi~5~\cite{raspberrypi5} running a retrained YOLOv8n detector~\cite{yolov8} (mAP50\,=\,0.995, 4.8\,FPS TFLite / 9\,FPS NCNN), a 20-state finite state machine~\cite{wagner2006fsm} on the ArduPilot autopilot~\cite{ardupilot}, and a lawnmower path planner with layered geofencing. A physics-based 216-configuration parameter sweep identified the optimal operating point (35\,m altitude, 8\,m/s, 70\textdegree{} scan angle), yielding $>$99.97\% single-pass detection probability at 10.9\% battery usage with a 1.5$\times$ NFZ safety margin. Video-replay testing confirmed GPS target estimation accuracy of CEP50\,=\,2.3\,m. Verification against twelve formal requirements shows 10~met and 2~partially met; a plus/delta evaluation identifies the gaps remaining before full autonomous flight.

% ════════════════════════════════════════════════════════════════════════
\section{Objective}\label{sec:df-objective}

The mission objective is to \textbf{locate a casualty as quickly as possible} within a defined search area, using autonomous aerial search with onboard computer vision~\citep{goodrich2008,lyu2023uav_sar_survey}.

% ════════════════════════════════════════════════════════════════════════
\section{Constraints}\label{sec:df-constraints}

Hard constraints that cannot be relaxed bound this objective and shape every downstream design choice:

\begin{itemize}[nosep]
  \item \textbf{No-Fly Zone (SSSI)} --- a protected ecological site~\citep{wca1981} borders the search area; entry is prohibited.
  \item \textbf{Flight area boundary} --- the search polygon is fixed by the mission brief (R01).
  \item \textbf{Altitude limit} --- 50\,m maximum per CAA regulations~\citep{ukcaa,cap722} (R03).
  ...
  \item \textbf{Hardware ceiling} --- onboard inference runs at approximately 9\,FPS with the deployed NCNN backend~\citep{ncnn} (YOLOv8n~\citep{yolov8} on Raspberry Pi~5~\citep{raspberrypi5}), with TFLite~\citep{tflite} at 4.8\,FPS available as a fallback.
\end{itemize}

% ════════════════════════════════════════════════════════════════════════
\section{Five Competing Dimensions}\label{sec:df-dimensions}

These constraints produce five competing performance dimensions that must be balanced:
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Time} --- the critical early hours of hypothermia survivability are ticking~\citep{golden2006hypothermia}.
...
Figure~\ref{fig:sensitivity-spider} shows the \emph{desired performance envelope}. The grey dashed polygon represents the theoretical best on each axis; the blue polygon shows what the selected configuration achieves. Safety and detection are pushed to the outer edge (non-negotiable), while time and energy sit lower (acceptable trade-offs). The envelope is deliberately \emph{not} circular: hard constraints define a minimum acceptable radius, while preferential objectives are optimised within whatever budget remains.
...
External factors shaped this priority ordering. Table~\ref{tab:steeple} maps each STEEPLE dimension to a concrete design consequence, showing how these factors influenced both the constraint hierarchy and specific technical choices such as operator-in-the-loop verification and single-class detection.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X l@{}}
    ...
    \midrule
    \textbf{S}ocial & Public trust in autonomous drones for rescue is growing but fragile~\citep{goodrich2007human}; a high-profile false positive or NFZ breach would set back adoption & Operator-in-the-loop verification; conservative confidence threshold (0.4) \\
    \textbf{T}echnological & Edge AI on SBCs now viable~\citep{edgeai} (YOLOv8n at 9\,FPS NCNN on Pi~5); consumer GPS accurate to $\sim$3\,m CEP95~\citep{misra2006global} & Hardware ceiling drives altitude/speed envelope; GPS error baked into NFZ margin \\
    ...
    \textbf{E}nvironmental & SSSI (Site of Special Scientific Interest) borders search area~\citep{wca1981}; bird nesting season possible & 30\,m NFZ buffer with 5-layer protection; waypoint filtering pre-flight \\
    \textbf{P}olitical & University flight permissions require demonstrated safety case~\cite{jarus2019sora}; CAA drone regulations apply & 50\,m altitude ceiling (R03); progressive flight testing protocol (6 steps) \\
    \textbf{L}egal & BVLOS operations restricted without specific exemptions~\citep{caa2024uas}; data protection for imagery & Visual line-of-sight operation; no image storage of bystanders; single-class detector \\
    \textbf{E}thical & Casualty may be in distress~\citep{golden2006hypothermia}; false negative delays rescue; false positive wastes critical time & Detection weighted highest (40\%); $>$99.97\% cumulative probability target \\
    \bottomrule
  \end{tabularx}
\end{table}
...
With the operational context established, we turn to a key architectural decision: the choice of inference backend for the Raspberry Pi~5~\citep{raspberrypi5,arm_a76}. This decision directly constrains the detection and energy dimensions. Table~\ref{tab:mcda-backend} presents a multi-criteria decision analysis (MCDA) comparing four viable options, each scored 1--5 and weighted by importance to the mission.
...
NCNN~\cite{ncnn} scored highest (4.4) due to its 4.5$\times$ speed advantage over TFLite~\cite{tflite} (72\,ms vs 327\,ms per frame, benchmarked on Pi~5) with no accuracy loss and minimal deployment overhead. TFLite (4.0) serves as a proven fallback. Ultralytics requires PyTorch (2\,GB+ dependencies, slow on ARM), while Hailo-8~\citep{hailo2023datasheet} offers hardware acceleration but requires a separate NPU board (\textsterling65+) with limited YOLOv8 support and a complex toolchain --- impractical for the project timeline. We adopted a dual-backend architecture: \texttt{vision.py} accepts a \texttt{backend} parameter, and model files are drop-in replacements with identical input/output tensor shapes (\texttt{[1,640,640,3]} / \texttt{[1,5,8400]}).

\textbf{Sensitivity.} The ranking holds under reasonable weight perturbations. NCNN retains first place even if inference speed weight is halved (0.15 instead of 0.30) --- its score drops to 3.95, still above TFLite (3.85) and Ultralytics (3.85). NCNN loses to TFLite only if deployment simplicity is weighted above 0.40, which would contradict the mission's time-critical detection requirement. This confirms that the backend choice is not an artefact of the weighting scheme.

% ════════════════════════════════════════════════════════════════════════
\section{Scoring the Five Dimensions}\label{sec:df-scoring}

With the inference backend selected, the hardware performance ceiling is fixed: 9\,FPS (NCNN) or 4.8\,FPS (TFLite fallback). Each axis of the spider chart (Figure~\ref{fig:sensitivity-spider}) corresponds to a normalised score $S \in [0,1]$, where 1 represents the theoretical best and 0 represents failure. Three of the five dimensions --- detection, coverage, and energy --- score continuously and combine into a composite objective for the 216-configuration sweep. The remaining two --- safety and time --- serve as \emph{pass/fail constraints}; the rationale for this asymmetry is explained at the end of this section.

% ── Safety ────────────────────────────────────────────────────────────
\subsection{Safety (\texorpdfstring{$S_\text{safety}$}{S\_safety})}\label{sec:score-safety}
...
\textbf{NFZ incursion risk.} The 30\,m buffer exceeds the RSS stopping distance (20.4\,m) by 1.5$\times$. An NFZ entry requires simultaneous failure of all five independent layers: (1)~geometric waypoint filtering, (2)~speed ramp deceleration, (3)~repulsive vector field at 3\,m, (4)~hard boundary mode switch, and (5)~pilot RC kill switch. The incursion probability is the product of five independent failure probabilities --- negligibly small for any realistic per-layer failure rate.
...
% ── Detection ─────────────────────────────────────────────────────────
\subsection{Detection (\texorpdfstring{$S_\text{detection}$}{S\_detection})}\label{sec:score-detection}
...
We use a multiplicative combination rather than a weighted sum because a zero in \emph{any} sub-component (e.g.\ zero frames on target, or target below minimum pixel size) should collapse the entire detection score to zero --- you cannot compensate for an invisible target by having a confident model. We exclude model mAP50 (0.995) from the composite product because it is constant across all configurations; it serves as a prerequisite validated in Step~1 of the decision chain.
...
% ── Time ──────────────────────────────────────────────────────────────
\subsection{Time (\texorpdfstring{$S_\text{time}$}{S\_time})}\label{sec:score-time}
...
% ── Energy ────────────────────────────────────────────────────────────
\subsection{Energy (\texorpdfstring{$S_\text{energy}$}{S\_energy})}\label{sec:score-energy}

Energy consumption is derived from a physics-based simulation~\citep{stolaroff2018energy} with three components:
...
\begin{enumerate}[nosep]
  ...
  \end{equation}
  This is a conservative upper bound: it assumes full deceleration to hover for each U-turn, whereas actual banked turns retain partial forward speed and therefore consume less energy. This ensures the energy budget is never underestimated.
\end{enumerate}
...
\end{equation}
where $E_\text{battery} = 115.4$\,Wh (6S 5200\,mAh LiPo). Our selected configuration uses $E_\text{total} = 12.6$\,Wh, giving $S_\text{energy} = 0.89$. A score of 1.0 means zero energy consumed (impossible); 0.0 means full battery depletion.

% ── Coverage ──────────────────────────────────────────────────────────
\subsection{Coverage (\texorpdfstring{$S_\text{coverage}$}{S\_coverage})}\label{sec:score-coverage}

Coverage is the fraction of the accessible search area~\citep{boustrophedon,covpath} that lies within at least one camera footprint during the scan:
...
The NFZ buffer removes approximately 4\% of the total search area from the accessible polygon. This is deliberate: the 30\,m buffer sacrifices a thin strip of coverage in exchange for guaranteed NFZ clearance. Our selected configuration achieves $S_\text{coverage} = 0.96$, meaning 96\% of the total area is covered and the remaining 4\% lies within the fenced reserve where casualty presence is unlikely given the NFZ protection status.
...
\textbf{What we did.} We retrained the YOLOv8n model~\citep{yolov8,yolohistory} on a mixed dataset of 300~synthetic images (using domain randomisation techniques~\citep{synthdata,domainrand}), 16~manually labelled real flight frames, and 50~hard-negative backgrounds at the camera's native 1456$\times$1088 resolution. Validation on the COCO-style metric~\citep{lin2014coco} yielded mAP50\,=\,0.995.
...
% ────────────────────────────────────────────────────────────────────────
\subsection{\texorpdfstring{Step 4: Determine Maximum Speed at 35\,m}{Step 4: Determine Maximum Speed at 35 m}}\label{sec:step4}
...
\textbf{The decision.} Lawnmower (boustrophedon) coverage is the standard method recommended by the International Aeronautical and Maritime Search and Rescue Manual (IAMSAR Vol.~III)~\cite{wca1981} and is widely deployed on autonomous marine and aerial SAR platforms~\cite{galceran2013survey,choset2001coverage}. The lawnmower pattern provided the best coverage guarantee for irregular polygons with NFZ constraints. The search area is a 5-corner irregular polygon with an SSSI no-fly zone on one edge --- not a convex shape where spiral patterns excel. The lawnmower pattern's deterministic lane structure means we can \emph{prove} that every accessible point is scanned, which is essential for a safety-critical SAR mission~\cite{murphy2014}. However, spiral inward patterns were also considered for their lower turn count. In a convex search area without NFZ constraints, a spiral would likely be the better choice.
...
\textbf{The energy model.} We applied the physics-based energy simulation from Section~\ref{sec:score-energy} (momentum-theory hover power plus quadratic parasitic drag) to every candidate angle. U-turns are modelled as decelerate--turn--reaccelerate sequences with energy cost proportional to the square of the entry speed; \emph{fewer U-turns} therefore translates directly to \emph{less energy}, and the scan angle that minimises U-turns is the one that aligns the scan lines with the polygon's longest edge.

\textbf{The 216-configuration sweep.} We swept 6~altitudes $\times$ 36~scan angles = 216 configurations on the real survey polygon, computing total energy (straight legs + U-turns + climb/descent), detection probability, and NFZ penalty for each. Figure~\ref{fig:energy-heatmap}: a clear low-energy valley runs through 65--75\textdegree{} at all altitudes.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{GPS position error}: $\pm$3\,m (consumer GPS CEP95~\citep{kaplan2017understanding}).
...
\textbf{The calculation.} At 35\,m altitude, the cross-track footprint is 34\,m. With 20\% overlap, adjacent lanes overlap by 6.8\,m. The maximum expected lane error (GPS + wind + control) is approximately 4.6\,m, leaving 2.2\,m of spare coverage. This ensures gap-free coverage under normal conditions.

\textbf{Why not more?} Higher overlap means more scan lines, more energy, and more time. We selected 20\% as the minimum that provides reliable gap-free coverage.
...
A physics-based simulation evaluated \textbf{216~configurations} (6~altitudes $\times$ 36~scan angles) on the real survey polygon, modelling energy consumption with momentum-theory hover power and quadratic parasitic drag, detection probability from the empirical detection envelope, and NFZ penalties for every combination.
...
Every parameter traces back to a physical measurement or regulatory constraint through the sequential chain detailed in Section~\ref{sec:df-chain}. Each value traces to measurement or regulation.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Operator-in-the-loop verification.} Requiring a human confirmation before descent reflects established human--robot interaction principles for safety-critical tasks~\citep{sheridan2002humans,endsley1999loa,cummings2014man_uav}. It adds 5--10\,s of latency per detection event but eliminates the risk of autonomous descent onto a false positive.
...
The mission controller implements a 20-state finite state machine~\citep{fsm,wagner2006fsm}:
...
Additional states handle edge cases: MANUAL (RC override), RETURN (RTL), BEACON\_REDIRECT (PLB signal integration), RESUME (post-override recovery with GPS fix and NFZ position validation), and several internal bookkeeping states. The RC override guard runs \emph{before} the state handler on every loop iteration: if the pilot switches away from GUIDED mode, all autonomous commands cease immediately and cannot resume until the pilot explicitly returns control~\citep{ardupilot_failsafe}. This prevents the software from ``fighting'' the pilot.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{state_machine_generated}
  \caption{Complete 20-state finite state machine. Green arrows show the nominal mission path; orange arrows show recovery transitions (manual override, resume, beacon redirect). Every state has an explicit exit to either the next nominal state or a safe fallback (MANUAL or RETURN).}\label{fig:state-machine}
\end{figure}
...
\begin{enumerate}[nosep]
  \item \textbf{Capture}: IMX296 global shutter sensor~\cite{imx296} at 1456$\times$1088 native resolution (BGR output, no colour conversion needed).
  \item \textbf{Undistort}: optional lens correction~\citep{lens_calibration} via precomputed \texttt{cv2.remap()} maps~\citep{opencv} (+1.5\,ms).
  ...
  \item \textbf{Post-process}: NMS~\citep{nms}, confidence threshold (0.4), bounding box extraction. Output: \texttt{(found, x, y, conf)}.
\end{enumerate}

We trained the model~\citep{yolov8} (mAP50\,=\,0.995) on 366 images: 300 synthetic composites at altitude-correct scale using domain randomisation~\citep{synthdata,domainrand}, 16 manually labelled real flight frames, and 50 hard-negative backgrounds~\citep{shrivastava2016training}, all at native 1456$\times$1088 resolution.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{detection_montage}
  \caption{Detection montage from DJI flight video at varying altitudes and lighting conditions. Each frame shows the bounding box, confidence score, and estimated altitude. The model reliably detects the rescue dummy from 15--50\,m, with confidence degrading gracefully as the target shrinks to fewer pixels at higher altitudes.}\label{fig:detection-montage}
\end{figure}

\subsection{Detection-to-GPS Pipeline}

When a detection is returned by the CV pipeline, the system converts the bounding-box centre $(c_x, c_y)$ into an estimated GPS coordinate in five steps: (i)~rescale from model space ($640\!\times\!640$) back to native frame coordinates ($1456\!\times\!1088$); (ii)~compute the pixel offset from the frame centre; (iii)~multiply by the Ground Sample Distance to obtain a metric offset in the camera frame; (iv)~rotate by drone yaw into a North/East offset; and (v)~project onto WGS-84 using the drone's GPS position. Multiple estimates across frames are fused via inverse-variance weighting~\cite{spagnolo2025kalman} and spatial clustering~\cite{barber2006vision,sun2016sar}.

The GSD links pixel displacement to ground distance:
\[
  \mathrm{GSD} = \frac{w_{\mathrm{sensor}} \cdot h}{f \cdot W_{\mathrm{px}}}
  = \frac{5.02\,\mathrm{mm} \times 35\,\mathrm{m}}{5.46\,\mathrm{mm} \times 1456\,\mathrm{px}}
  = 0.0221\;\mathrm{m/px}
\]

\noindent giving a field of view of $32.2 \times 24.0$\,m at the 35\,m operating altitude.

\begin{table}[H]
\centering\small
\caption{Error budget for detection-to-GPS estimation~\citep{gautam2018error}.}
\label{tab:error-budget}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Source} & \textbf{Magnitude} & \textbf{When} \\
\midrule
Bbox regression (model) & $\pm$6.8\,px ($\pm$0.15\,m) & Always \\
Altitude uncertainty (baro) & $\pm$1.4--2.9\,\% GSD & Always \\
Compass / yaw & $\pm$0.35\,m & Always \\
Drone GPS position & $\pm$2--3\,m (dominant) & Always \\
Roll / pitch tilt & 0--6\,m$^\dagger$ & Flight only \\
\bottomrule
\end{tabular}

{\footnotesize $\dagger$~Reduced to ${\sim}0.3$\,m with attitude compensation enabled (\texttt{-{}-compensate-tilt}).}
\end{table}

Without correction, forward flight pitch introduces up to 6.2\,m offset ($h\tan\theta = 35\tan 10^{\circ}\approx 6.2$\,m)---larger than all other error sources combined.  Attitude-compensated estimation (\texttt{-{}-compensate-tilt}) reduces this to ${\sim}0.3$\,m by ray-tracing each detection through the full Tait--Bryan rotation matrix~\citep{beard2012small} $\mathbf{R} = \mathbf{R}_z(\psi)\,\mathbf{R}_y(-\theta)\,\mathbf{R}_x(-\phi)$ constructed from the drone's \texttt{ATTITUDE} telemetry via MAVLink~\citep{mavlink}, which is the exact mathematical inverse of the camera model used in simulation.  We verified this with 1440 automated test cases across all altitudes and tilt angles, each producing errors below 0.01\,m.  The centering procedure further mitigates residual error by hovering ($\theta < 1^{\circ}$) before the final landing estimate.

Field testing with DJI flight video replay yielded \textbf{CEP50\,=\,2.3\,m} (50\% of estimates within 2.3\,m of the true position). The primary residual error source is GPS receiver latency (100--200\,ms), causing ${\sim}1$\,m along-track offset at 5\,m/s; multi-pass averaging from different headings cancels this systematic bias.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\linewidth]{bullseye_comparison}
  \caption{GPS target estimation accuracy from DJI video replay. Left: raw single-frame estimates showing along-track spread caused by GPS receiver latency. Right: after inverse-variance weighted averaging and spatial clustering, the estimate converges to CEP50\,=\,2.3\,m. Bullseye rings at 2, 5, and 10\,m.}\label{fig:bullseye}
\end{figure}

\subsubsection{Localization Approach Selection}

We surveyed ten localization approaches~\citep{barber2006vision,paulin2024raycast,sambolek2025person}, spanning single-frame direct georeferencing, multi-frame triangulation, Kalman filtering~\citep{kalman1960}, visual odometry/SLAM, optical flow, homography decomposition, terrain-referenced navigation, multi-UAV cooperative estimation, RTK-GPS~\citep{takasu2009development}, and photogrammetric bundle adjustment.  We evaluated each against three constraints: real-time operation at 3--5\,FPS on a Raspberry Pi~5, no additional hardware beyond the standard autopilot sensor suite (GPS, barometer, compass), and graceful degradation if the drone cannot hover.  The selected combination---direct georeferencing with attitude compensation, fused via inverse-variance weighting (IVW) and spatial clustering---operates on single frames (no history bootstrapping), runs in $<$0.1\,ms per estimate, and naturally handles variable-quality observations by weighting near-centre detections more heavily than edge detections.

\subsubsection{Progressive Accuracy Improvement}

The mission architecture exploits the state machine to progressively eliminate error sources as the drone transitions from search to landing.  Table~\ref{tab:accuracy-progression-body} summarises the four phases.

\begin{table}[H]
  \centering\small
  \caption{Target position accuracy progression through the detection-to-landing flow.}\label{tab:accuracy-progression-body}
  \begin{tabular}{@{}llcc@{}}
    \toprule
    \textbf{Phase} & \textbf{Drone condition} & \textbf{CEP$_{50}$} & \textbf{Dominant error} \\
    \midrule
    SEARCH (detection)    & 5\,m/s, ${\sim}10^{\circ}$ pitch  & ${\sim}3.5$\,m & Tilt projection \\
    CENTERING (approach)  & Decelerating $\to$ hover           & ${\sim}2.3$\,m & GPS receiver noise \\
    CENTERING (hover lock)& Stationary, nadir camera           & ${\sim}1.8$\,m & GPS averaging limit \\
    Tilt-compensated      & Any phase with \texttt{ATTITUDE}   & ${\sim}0.3$\,m & Residual calibration \\
    \bottomrule
  \end{tabular}
\end{table}

\noindent The key insight is that centering the target in the frame and hovering eliminates the largest error sources \emph{without software correction}: when the target is at the optical centre, pixel offset is zero, so every multiplicative error in the projection chain (GSD, focal length, yaw, lens distortion) is multiplied by zero and vanishes.  The only remaining error is the drone's own GPS position, which dominates the budget at 60--70\% of total variance across all phases.  Figure~\ref{fig:bullseye-comparison-body} shows the progressive tightening across estimation strategies.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{bullseye_comparison}
  \caption{Bullseye comparison of four estimation strategies on DJI replay data.  Left to right: single-frame (CEP$_{50}$\,=\,5.1\,m), rolling average, IVW clustering (2.3\,m), and hover-lock (1.8\,m).  The progressive tightening of the scatter cloud demonstrates the value of multi-frame fusion and centering.}\label{fig:bullseye-comparison-body}
\end{figure}

\subsubsection{Frame Rate Sufficiency}

At 35\,m altitude the camera footprint is $32.2 \times 24.0$\,m, so a target at the strip centreline remains visible for $24.0/5 = 4.8$\,s at the 5\,m/s search speed.  At 4.8\,FPS (Pi~5 TFLite), this yields ${\sim}23$ inference frames per pass with a per-frame detection probability $p_d \approx 0.78$ (measured from DJI replay), giving ${\sim}18$ expected detections.  The probability of obtaining zero detections in a single pass is $(1 - 0.78)^{23} < 10^{-15}$, and the probability of at least 5 detections (the SMART clustering threshold) exceeds 99.9\%.  A single flyover therefore provides sufficient data for a reliable position lock.
...
\subsection{Safety Architecture}\label{sec:safety-arch}

The survey area is adjacent to a Site of Special Scientific Interest (SSSI), making boundary enforcement a hard safety constraint. The system implements a \textbf{five-layer geofence} (Figure~\ref{fig:geofence_layers}), where each layer operates independently so that failure of any single layer is caught by the next:

\begin{enumerate}[nosep]
  \item \textbf{Speed scalar field} (20--2\,m from SSSI): linearly reduces maximum approach speed to zero near the boundary, allowing full-speed parallel flight via directional clamping.
  \item \textbf{Repulsive vector field} ($<$\,3\,m): applies a 5\,m/s push perpendicular to the nearest boundary segment, exceeding the near-zero speed cap to prevent penetration.
  \item \textbf{Hard boundary cutoff}: if GPS places the aircraft inside the SSSI polygon (\texttt{cv2.pointPolygonTest}), all commands halt and the system transitions to manual mode.
  \item \textbf{Plan-time waypoint filter}: removes any search waypoints within 30\,m of the NFZ during pattern generation, preventing the planner from routing near the boundary.
  \item \textbf{Firmware geofence}: an independent Cube Orange~\citep{cubepilot} polygon fence triggers RTL if all software layers fail---this operates at the flight-controller level and cannot be overridden by companion-computer code.
\end{enumerate}

\noindent A separate Flight Area containment check triggers emergency RTL if the aircraft exits the 4-vertex operational boundary; this is independent of the SSSI geofence and cannot be disabled.

The \textbf{RC override guard} is the first check on every main-loop iteration: if the pilot switches the transmitter away from GUIDED or LAND mode, \emph{all} MAVLink commands are suppressed on the same loop tick. A real incident motivated this design (a colleague's drone crashed when software fought the pilot's RC input) and ensures the pilot can always regain control with a single switch. Additional safety features include a \textbf{50\,m altitude hard cap} enforced at the navigation layer (every \texttt{send\_global\_target()} call is clamped), \textbf{link-loss RTL} via ArduCopter's GCS failsafe~\citep{ardupilot_failsafe} when heartbeats cease, and an \textbf{RC kill switch} that returns the aircraft to manual STABILIZE mode independently of all software.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.80\linewidth]{geofence_layers}
  \caption{Five-layer geofence shown to scale on the Fenswood Farm site. From outermost to innermost: Flight Area hard boundary, 30\,m waypoint buffer, 20\,m speed scalar field, 3\,m repulsive vector field, and SSSI hard cutoff. Graduated protection ensures that even if one layer fails, subsequent layers prevent boundary incursion.}\label{fig:geofence_layers}
\end{figure}
...
With 10 of 12 requirements met and two partially met, the system demonstrates simulation-level capability but has not yet completed autonomous outdoor flight. This section examines what worked, what would change, and where the primary risks remain.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Physics-based optimisation.} The 216-configuration sweep with a momentum-theory energy model replaced guesswork with quantified trade-offs. The Pareto analysis confirmed that the selected point is optimal, not merely ``good enough.''
  \item \textbf{Progressive testing protocol.} The 6-step flight test ladder~\citep{nasa2015vv,koopman2016challenges} (bench commands $\to$ passive CV $\to$ waypoints $\to$ auto detect $\to$ full mission) meant each step added exactly one new risk.
  \item \textbf{Retrained model quality.} Mixed synthetic + real + negative training at native resolution produced mAP50\,=\,0.995 --- a detector that validated as the foundation for all altitude/speed decisions.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{GPS timing lag compensation.} The 100--200\,ms GPS latency causes 1\,m along-track error at 5\,m/s, visible as diagonal spread in target estimates (max error 16.5\,m). We designed a velocity-based offset correction but did not implement it. This would improve CEP50 from 2.3\,m toward sub-metre accuracy.
...
We rate simulation capability at 7.7/10 (all states exercised, energy model validated, detection pipeline proven) but only 2.1/10 in real-world readiness: bench and video-replay testing are complete, yet no autonomous outdoor flight has been conducted. This 2.1 reflects the absence of Stages~6--7 (manual passive flight and full autonomous mission) in the simulation-to-real progression. The gap between simulation confidence and field-proven reliability is the project's primary risk, driven by weather cancellations and scheduling constraints rather than technical blockers. The optimisation framework is re-evaluable: if field data shifts the detection envelope, the entire chain can be re-run in minutes to produce updated parameters.



\subsection{Comparison with Published SAR Systems}\label{sec:eval-comparison}

Table~\ref{tab:sar-comparison} positions this project against published SAR drone systems~\citep{lyu2023uav_sar_survey,mishra2020}. Direct comparison is imperfect---differing hardware, environments, and target types---but provides context that internal metrics alone cannot.

\begin{table}[H]
  \centering\small
  \caption{Performance comparison with published SAR systems. Our results are from bench/video proxy (no outdoor flight); published results are from outdoor flights.}\label{tab:sar-comparison}
  \begin{tabular}{@{}p{2.4cm}p{2.0cm}p{1.7cm}p{1.2cm}p{1.3cm}p{2.2cm}p{1.8cm}@{}}
    \toprule
    \textbf{System} & \textbf{Platform} & \textbf{Detector} & \textbf{FPS} & \textbf{mAP$_{50}$} & \textbf{GPS Acc.} & \textbf{Est.\ Cost} \\
    \midrule
    This project & Pi~5 CPU & YOLOv8n & 4.8$^a$ & 0.995$^b$ & CEP$_{50}$\,=\,2.3\,m$^c$ & \pounds800 \\
    AUSPEX~\cite{auspex2025} & Jetson Orin & YOLOv5s & 30 & 0.89 & $<$5\,m (RTK) & $>\$5\,\text{k}$ \\
    Sambolek~\cite{sambolek2021} & Jetson TX2 & YOLOv3-tiny & 22 & 0.81 & --- & $>\$3\,\text{k}$ \\
    SearchWing~\cite{searchwing2024} & Comp.\ PC & Custom CNN & 5 & 0.78 & $<$10\,m & ${\sim}\$2\,\text{k}$ \\
    \bottomrule
    \multicolumn{7}{@{}l}{\footnotesize $^a$TFLite; 9\,FPS with NCNN. \;$^b$Upper bound; realistic ${\sim}$0.85--0.90 (87\% video detection rate).} \\
    \multicolumn{7}{@{}l}{\footnotesize $^c$DJI video replay, not live flight.} \\
  \end{tabular}
\end{table}

The system is competitive on detection accuracy (mAP$_{50}$\,$\geq$\,0.89 after realistic adjustment) and GPS estimation (CEP$_{50}$\,=\,2.3\,m vs.\ $<$5\,m for RTK-aided AUSPEX~\cite{auspex2025}), while operating at roughly one-fifth the hardware cost by using a \pounds35 Raspberry Pi~5 rather than an NVIDIA Jetson~\citep{edgeai,david2021tensorflow}. The primary deficit is inference speed: 4.8\,FPS on CPU versus 22--30\,FPS on GPU platforms~\citep{sambolek2021,auspex2025,realtimesar2025}. This was mitigated by the NCNN backend~\cite{ncnn} (9\,FPS) and the altitude/speed optimisation in Section~\ref{sec:step4}, which ensures the slower frame rate does not reduce cumulative detection probability. SearchWing~\cite{searchwing2024}, the closest analogue in cost and compute class, achieves similar FPS but lower accuracy, suggesting that the mixed real+synthetic retraining strategy (Section~\ref{sec:step1}) provides a meaningful advantage even on constrained hardware. Figure~\ref{fig:radar_comparison} presents a radar comparison across six capability dimensions.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\linewidth]{radar_comparison}
  \caption{Multi-dimensional comparison against published SAR platforms (Table~\ref{tab:sar-comparison}). This project scores highest on cost-effectiveness and test coverage but lowest on inference speed due to CPU-only deployment.}\label{fig:radar_comparison}
\end{figure}

% ════════════════════════════════════════════════════════════════════════
\section{Evidence-Based Adaptation}\label{sec:decisions-changed}

Four significant corrections were made during development --- each discovered through systematic testing, each requiring a different diagnostic approach, and each validating the modular architecture that isolated the fix to a single module.

\subsection{BGR Colour Channel Fix}

The IMX296 global shutter sensor~\citep{imx296} outputs BGR-ordered bytes despite picamera2~\citep{picamera2} labelling the format as \texttt{RGB888}. Our initial code included \texttt{cv2.cvtColor(frame, cv2.COLOR\_RGB2BGR)}, which double-swapped the red and blue channels, producing a persistent blue tint on all camera images. Standard debugging (adjusting white balance, colour gains, ISP tuning) failed because the raw data was already correct --- the conversion was the problem.
...
Total power at forward speed~$v$ comprises three terms~\citep{johnson2013helicopter}:
...
The 58 test scripts and 127 unit test functions are integral to the optimisation methodology~\citep{forsberg1991relationship}, providing the empirical evidence that anchors every link in the decision chain.
...
The system follows a deliberate progression from simulation to reality, where each stage adds exactly one new variable. This approach is standard practice in autonomous systems development~\citep{ardupilot2024,nasa2015vv,forsberg1991relationship} and ensures that failures at any stage can be attributed to the newly introduced variable rather than an unknown interaction.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Pi + camera, no Cube} (completed): camera capture, AI inference, MJPEG streaming~\citep{rfc2435} on real hardware. Validates: IMX296 integration, BGR colour order, inference speed on Pi~5 (206.5\,ms confirmed).
...
Each stage produced quantitative data that fed back into the optimisation chain: Stage~4 established the 4.8\,FPS inference rate used in Step~4 of the decision chain; Stage~5 confirmed the mavproxy~\citep{mavproxy} bridge architecture; the DJI video replay (a parallel validation track using real flight footage from a manual DJI flight) provided the detection envelope data used in Steps~2--3 and the GPS estimation accuracy (CEP50\,=\,2.3\,m) reported in Section~\ref{sec:df-result}.
...
Four mid-development corrections --- BGR colour fix, focal length recalibration, TFLite coordinate bug, and calibration corruption --- demonstrated that the modular architecture confined errors to single files without ripple effects. The 58 test scripts and 127 unit tests provided the empirical foundation for every link in the decision chain, while the seven-stage simulation-to-real progression introduced each new variable incrementally.

The framework can be re-evaluated in minutes as field data replaces simulation assumptions, closing the gap between demonstrated simulation capability (7.7/10) and the outdoor flight validation (2.1/10) that remains the next critical milestone.
