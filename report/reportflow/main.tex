...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Undistort}: optional lens correction via precomputed \texttt{cv2.remap()} maps~\cite{opencv} (+1.5\,ms).
...
We trained the model~\cite{yolov8} (mAP50\,=\,0.995) on 366 images: 300 synthetic composites at altitude-correct scale~\cite{synthdata}, 16 manually labelled real flight frames, and 50 hard-negative backgrounds, all at native 1456$\times$1088 resolution.
...
\begin{table}[H]
\centering\small
\caption{Error budget for detection-to-GPS estimation.}
...
Without correction, forward flight pitch introduces up to 6.2\,m offset ($h\tan\theta = 35\tan 10^{\circ}\approx 6.2$\,m)---larger than all other error sources combined.  Attitude-compensated estimation (\texttt{-{}-compensate-tilt}) reduces this to ${\sim}0.3$\,m by ray-tracing each detection through the full rotation matrix $\mathbf{R} = \mathbf{R}_z(\psi)\,\mathbf{R}_y(-\theta)\,\mathbf{R}_x(-\phi)$ constructed from the drone's \texttt{ATTITUDE} telemetry, which is the exact mathematical inverse of the camera model used in simulation.  We verified this with 1440 automated test cases across all altitudes and tilt angles, each producing errors below 0.01\,m.  The centering procedure further mitigates residual error by hovering ($\theta < 1^{\circ}$) before the final landing estimate.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Firmware geofence}: an independent Cube Orange polygon fence triggers RTL if all software layers fail---this operates at the flight-controller level and cannot be overridden by companion-computer code.
\end{enumerate}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Progressive testing protocol.} The 6-step flight test ladder (bench commands $\to$ passive CV $\to$ waypoints $\to$ auto detect $\to$ full mission) meant each step added exactly one new risk.
