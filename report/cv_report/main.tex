...
Fifty frames were extracted from flight video at locations where no mannequin was present. Each negative image is paired with an empty label file. The v1 model (trained without negatives) frequently hallucinated detections on shadows, path markings, and field equipment. After retraining with the negative set, false activations on these terrain features were reduced while maintaining high recall on the mannequin target. The negative-to-positive ratio of 50:316 (${\sim}$16\%) follows the heuristic recommended by~\textcite{shrivastava2016training}.
...
All three backends consume the same model file and produce output through the identical \texttt{detect\_in\_image(frame)} $\to$ \texttt{(found, cx, cy, confidence)} interface. The calling code---state machine, GPS estimation, ground station, and all 58 test scripts---is backend-agnostic. This architecture provides three concrete benefits: (1)~development--production parity, where detections in laptop simulation match Pi behaviour; (2)~graceful degradation, where a missing backend triggers automatic fallback rather than a crash; and (3)~A/B testing capability, where backends can be compared on identical input frames via a single flag.
...
\begin{table}[ht]
  \centering
  \caption{Inference backend comparison for YOLOv8n ($640 \times 640$ input) on Raspberry Pi~5. Projected values are marked with $\dagger$.}\label{tab:backend-comparison}
  \begin{tabular}{@{}l c c c c@{}}
    ...
    NCNN$^\dagger$      & ${\sim}$68\,ms  & ${\sim}$15\,FPS & 3.1\,MB  & \texttt{ncnn} (C++ + Python) \\
...
\begin{table}[ht]
  \centering
  \caption{Raw inference vs effective pipeline throughput on Raspberry Pi~5. \emph{Italic} rows indicate projected values.}\label{tab:pipeline-fps}
  \begin{tabular}{@{}l c c c c l@{}}
    ...
    \textit{TFLite FP16 (XNNPACK)} & \textit{${\sim}$100\,ms} & \textit{${\sim}$10} & \textit{+12\,ms} & \textit{${\sim}$8.9} & \textit{Projected} \\
    \textit{NCNN FP32}              & \textit{${\sim}$77\,ms}  & \textit{${\sim}$13} & \textit{+15\,ms} & \textit{${\sim}$10.9} & \textit{Benchmarked} \\
...
A pre-trained YOLOv8n model exported from the standard COCO dataset~\cite{lin2014coco} was retained as a flight-day backup (\texttt{models/human.tflite}, 13\,MB, 80 classes). This model detects people without any SAR-specific training, providing a low-effort fallback if the custom model proved unreliable in field conditions.
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}l l c c l@{}}
    ...
    \textit{TFLite FP16 (XNNPACK)}    & \textit{Projected} & \textit{${\sim}$100\,ms}  & \textit{${\sim}$10}   & \textit{Native A76 FP16 FMLA} \\
    \textit{NCNN FP32 (4 threads)}    & \textit{Projected} & \textit{${\sim}$68\,ms}   & \textit{${\sim}$15}   & \textit{Ultralytics benchmark} \\
    \textit{NCNN FP16}                & \textit{Projected} & \textit{${\sim}$45\,ms}   & \textit{${\sim}$22}   & \textit{NEON FP16 + NCNN kernels} \\
    \textit{Hailo-8L NPU}             & \textit{Hardware}  & \textit{$<$15\,ms} & \textit{$>$60}   & \textit{External accelerator} \\
    \bottomrule
  \end{tabular}
\end{table}
...
With the model trained and the inference pipeline deployed, the engineering question shifts from ``can it detect?'' to ``under what conditions does it detect reliably?'' This section characterises the detection system's operational envelope by deriving the relationships between altitude, speed, target pixel size, motion blur, and frame coverage from the calibrated hardware parameters (Table~\ref{tab:vision-params}).
...
The IMX296 is a global shutter sensor, eliminating rolling-shutter distortion. Translational motion blur remains a function of exposure time:
...
At all operational speeds, blur is sub-pixel with the global shutter at 1/1000\,s. Even at \SI{15}{\metre\per\second} and \SI{20}{\metre} altitude, blur reaches only 1.2\,px. The global shutter is an important hardware choice: a rolling-shutter sensor would introduce 5--10\,px of geometric distortion at typical multirotor vibration frequencies.
...
Even in the worst operational case (50\,m at 15\,m/s with conservative 70\% single-frame detection), the cumulative detection probability exceeds 99.8\%. At nominal conditions (35\,m, 8\,m/s), the calculated detection probability exceeds 99.99\% with 14.5 frames per target. The lawnmower pattern provides additional redundancy: adjacent passes overlap by 20\%, so the probability of missing a target across two overlapping passes is $(1 - 0.9997)^2 \approx 10^{-7}$.
...
\begin{tikzpicture}[
  ...
  imgbox/.style={draw, rounded corners=2pt, fill=blue!8, minimum width=3.4cm, minimum height=0.55cm, align=center, font=\scriptsize\sffamily},
  nnbox/.style={draw, rounded corners=2pt, fill=green!12, minimum width=3.4cm, minimum height=0.55cm, align=center, font=\scriptsize\sffamily},
  coordbox/.style={draw, rounded corners=2pt, fill=orange!10, minimum width=3.4cm, minimum height=0.55cm, align=center, font=\scriptsize\sffamily},
  valbox/.style={draw, rounded corners=2pt, fill=red!8, minimum width=3.4cm, minimum height=0.55cm, align=center, font=\scriptsize\sffamily},
...
\node[decision, below=0.2cm of s1] (d1) {calib data\\exists?};
\node[imgbox, below=0.2cm of d1] (s2) {2. Lens undistortion\\[-1pt]{\tiny cv2.remap, +1.5\,ms}};
\draw[arr] (s1) -- (d1);
\draw[arr] (d1) -- node[right, annot]{Yes} (s2);
\draw[arr] (d1.east) -- ++(0.9,0) |- node[near start, above, annot]{No (skip)} (s2.east);

% ── Preprocessing ──
\node[imgbox, below=0.2cm of s2] (s3) {3. Resize 640$\times$640\\[-1pt]{\tiny squish, $s_x$=0.44, $s_y$=0.59}};
\node[imgbox, below=0.2cm of s3] (s4) {4. BGR$\to$RGB + normalise\\[-1pt]{\tiny float32 $[0,1]$, shape $[1,640,640,3]$}};
\draw[arr] (s2) -- (s3);
\draw[arr] (s3) -- (s4);

% ── Inference ──
\node[nnbox, below=0.2cm of s4] (s5) {5. YOLOv8n inference\\[-1pt]{\tiny $[1,5,8400]$, 196\,ms TFLite / 68\,ms NCNN}};
\draw[arr] (s4) -- (s5);

% ── Post-processing ──
\node[decision, below=0.2cm of s5] (d2) {conf $>$ 0.2?};
\node[coordbox, below=0.2cm of d2] (s7) {7. Rescale to 1456$\times$1088\\[-1pt]{\tiny $\times$2.275 horiz, $\times$1.700 vert}};
\draw[arr] (s5) -- (d2);
\draw[arr] (d2) -- node[right, annot]{Yes} (s7);
\node[annot, right=0.6cm of d2] (discard) {Discard};
\draw[arr] (d2.east) -- (discard);

\node[coordbox, below=0.2cm of s7] (s8) {8. Pixel offset from centre\\[-1pt]{\tiny $\Delta x = u - 728$, $\Delta y = v - 544$}};
\node[coordbox, below=0.2cm of s8] (s9) {9. GSD calculation\\[-1pt]{\tiny $0.0221$\,m/px at 35\,m}};
\node[coordbox, below=0.2cm of s9] (s10) {10. Camera-frame offset\\[-1pt]{\tiny $d_\text{fwd}$, $d_\text{right}$ in metres}};
\node[coordbox, below=0.2cm of s10] (s11) {11. Yaw rotation\\[-1pt]{\tiny camera $\to$ NED frame}};
\node[coordbox, below=0.2cm of s11] (s12) {12. WGS\,84 projection\\[-1pt]{\tiny $\Delta\phi$, $\Delta\lambda$ from drone GPS}};
\draw[arr] (s7) -- (s8);
\draw[arr] (s8) -- (s9);
\draw[arr] (s9) -- (s10);
\draw[arr] (s10) -- (s11);
...
% ── Validation ──
\node[decision, below=0.2cm of s12] (d3) {passes\\filters?};
\node[valbox, below=0.2cm of d3] (s15) {15. Enqueue target\\[-1pt]{\tiny $(\phi, \lambda, \text{conf})$ $\to$ CENTERING}};
\draw[arr] (s12) -- (d3);
...
\node[errlabel, right=1.4cm of s1] {$\pm$2\,px sensor noise};
\node[errlabel, right=1.4cm of s5] {$\pm$3\,px bbox regression};
\node[errlabel, right=1.4cm of s7] {$\pm$6.8\,px rescaled};
\node[errlabel, right=1.4cm of s9] {$\pm$1.4\% (baro drift)};
\node[errlabel, right=1.4cm of s11] {$\pm$0.35\,m (compass)};
\node[errlabel, right=1.4cm of s12] {$\pm$2--3\,m (GPS noise)};

% ── Telemetry inputs (left side) ──
\node[annot, left=0.8cm of s9] (tel1) {altitude $h$};
\node[annot, left=0.8cm of s11] (tel2) {heading $\psi$};
\node[annot, left=0.8cm of s12] (tel3) {$\phi_d, \lambda_d$};
...
% ── Legend ──
\node[below=0.4cm of s15, font=\scriptsize\sffamily] (leg) {%
...
\begin{itemize}
    ...
    \item The v3 model outperformed v1 on the same video: v1 produced 12 false positives across the flight, while v3 produced zero
\end{itemize}
