...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}lccl@{}}
    ...
    Roll/pitch attitude    & \SI{3.1}{\metre} & \SI{1.3}{\metre} & $5\text{--}10^{\circ}$ in flight; ${\sim}0^{\circ}$ hovering \\
    ...
    \midrule
    \textbf{RSS (in flight)}   & \textbf{\SI{4.2}{\metre}} & \textbf{\SI{2.7}{\metre}} & Including roll/pitch \\
...
\subsection{Roll/Pitch Attitude Error}\label{sec:cv:rollpitch}

The GPS projection assumes the camera points straight down (nadir).  In practice, multirotor drones tilt to accelerate: forward flight requires pitch, lateral movement requires roll.  The resulting ground footprint displacement is

\begin{equation}\label{eq:tilt}
  \Delta_\mathrm{tilt} = h \cdot \tan(\theta)
\end{equation}

\noindent where $\theta$ is the tilt angle from nadir.  This error is \emph{not compensated} in the current implementation.  Figure~\ref{fig:tilt-geometry} illustrates the geometry.
...
\begin{table}[ht]
  \centering
  \caption{Ground displacement from roll/pitch tilt.}\label{tab:tilt-error}
  \begin{tabular}{@{}rccl@{}}
    \toprule
...
At \SI{35}{\metre} with $10^{\circ}$ pitch, the footprint shifts \SI{6.2}{\metre}---larger than GPS noise and all other error sources combined.  This is the single largest uncompensated error during forward flight.

\paragraph{Why the system tolerates this error.}  Three mechanisms mitigate the roll/pitch contribution:

\begin{enumerate}
  \item \textbf{State-dependent exposure.}  The error is present only during SEARCH (forward flight).  During CENTERING and VERIFY, the drone hovers ($\theta < 1^{\circ}$), reducing $\Delta_\mathrm{tilt}$ to $< \SI{0.3}{\metre}$.  The final position estimate used for approach and landing is computed primarily from hovering observations.

  \item \textbf{Multi-pass averaging.}  The lawnmower pattern causes alternating-direction flyovers.  Pitch displaces estimates in opposite directions on adjacent passes, partially cancelling through weighted averaging.

  \item \textbf{Altitude-dependent weighting.}  The inverse-variance weight $w_\mathrm{alt} = (h_\mathrm{ref}/h)^2$ gives low-altitude hovering observations $5\times$ the weight of high-altitude search observations.  The tilt-contaminated search observations are naturally suppressed.
\end{enumerate}

\paragraph{Potential future correction.}  The ArduPilot \texttt{ATTITUDE} MAVLink message provides roll and pitch at 10\,Hz.  A rotation-matrix correction could reduce the in-flight error from \SI{6.2}{\metre} to ${\sim}$\SI{0.5}{\metre} (limited by IMU noise of $\pm 0.5^{\circ}$).  This was not implemented because: (1)~the centering procedure already eliminates the error for the critical landing estimate; (2)~temporal synchronisation between telemetry and camera frames to within ${\sim}$\SI{50}{\milli\second} is required but not yet architected; and (3)~the additional complexity in a safety-critical path was judged unjustified given the operational mitigation.  This remains the highest-impact future improvement for the localisation subsystem.
