...
\subsection{Three Dataset Generations}
...
\textbf{v1} (\texttt{sar\_640}) was the initial baseline, trained exclusively on 200 synthetic composite images generated at $640 \times 640$ from a single satellite background. It demonstrated proof-of-concept detection in simulation but exhibited frequent false positives on terrain features---shadows, path markings, and field equipment---due to the absence of negative examples and limited background diversity.

\textbf{v2} (\texttt{sar\_1280}) increased training resolution to $1280 \times 1280$, yielding a marginal mAP improvement. However, the training data remained purely synthetic from the same single background source, so the false-positive problem persisted. The key lesson: \emph{resolution alone cannot compensate for limited data diversity}.

\textbf{v3} (\texttt{sar\_v2\_1088}) addressed both limitations simultaneously. Training resolution was set to $1088 \times 1088$ (matching the native camera sensor height), and the dataset was restructured with three complementary sources: 300 synthetic composites using real flight-video backgrounds, 16 manually labelled real aerial frames, and 50 negative frames. This combination produced mAP$_{50}$ = 0.995 and, critically, near-zero false positives during video replay analysis across 15--50\,m altitude.
...
At the deployed 4.8 effective FPS, inference consumes 94.2\% of the frame budget. The \SI{12}{\milli\second} pipeline overhead is negligible---pipeline optimisation (threaded capture, asynchronous GPS estimation) would yield only marginal improvement. The system is unambiguously \emph{inference-bound}. At NCNN speeds, overhead rises to 16.3\% of total frame time, making threaded capture worthwhile.
...
This section characterises the detection system's operational envelope by deriving the relationships between altitude, speed, target pixel size, motion blur, and frame coverage from the calibrated hardware parameters.
...
\subsection{Combined Detection Envelope}

The pixel size, blur, and frame count analyses are combined into a single scalar detection score:
...
Accurate target localisation depends on the fidelity of every link in the imaging chain. Four calibration campaigns were conducted, each correcting a specific source of systematic error.
...
\subsection{Camera Colour-Space Discovery}

The IMX296 is configured via \texttt{picamera2} with the \texttt{RGB888} format, documented as returning RGB data. However, the sensor actually outputs \textbf{BGR} byte order. This was confirmed by testing all six channel permutations: only the identity permutation (treating data as BGR without conversion) produced correct colours.

The fix was to remove the incorrect \texttt{cv2.cvtColor(frame, cv2.COLOR\_RGB2BGR)} call entirely. Impact: (1) the YOLO model receives correctly ordered data, restoring confidence to $>$0.95; (2) the MJPEG stream displays natural colours; (3) \SI{1.2}{\milli\second} per-frame latency saved.
...
\subsection{DJI Video Replay Analysis}

In the absence of full autonomous flight testing, detection performance was validated using DJI Mini~3 Pro flight recordings at the Fenswood Farm site. The DJI footage was centre-cropped from $3840 \times 2160$ to $1456 \times 1088$ to match the onboard camera's aspect ratio. Synchronised SRT telemetry provided frame-accurate GPS coordinates and altitude.

\textbf{Critical SRT sync issue.} The DJI SRT file contains 6854 entries (one per 30\,fps frame). Downsampled 6\,fps video (1370 frames) does not align with SRT entries---frame $N$ in 6\,fps maps to the wrong SRT entry. All analysis uses the 30\,fps video exclusively.

The video analysis pipeline (\texttt{tests/laptop/video\_test.py}) processes each frame through the identical detection and GPS estimation pipeline used in the production system. Results across the 15--50\,m altitude range confirm reliable detection with the v3 model, zero false positives on background terrain, and GPS estimation convergence consistent with the theoretical error budget.
...
These improvements are ordered by implementation effort: items 1--2 require only additional data or a flag change, items 3--4 require packaging or architecture evaluation, and items 5--6 require new hardware or dataset restructuring. The modular vision architecture ensures any improvement can be integrated without modifying the state machine, ground station, or flight control code.
