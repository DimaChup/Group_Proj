...
\begin{description}
    \item[\texttt{tests/hardware/}] \emph{Component-level checks} (8~scripts).  Each script tests a single peripheral---camera, AI model (TFLite and NCNN backends), GPS receiver, or buzzer---in isolation.  All issue zero flight commands and are safe to run on the bench at any time.
    \item[\texttt{tests/flight/}] \emph{Progressive flight tests} (14~scripts, numbered 0a--5).  These form the critical path from bench to autonomous flight.  Six bench scripts (0a--0f) validate the command pipeline, planning, geofence, and servo without propellers; five airborne scripts (1--5) build trust incrementally from manual RC with passive logging through to full autonomous search-and-land; plus geofence verification and map utilities.
    ...
    \item[\texttt{tests/laptop/}] \emph{Development-only tools} (21~scripts).  Model inspection, TFLite and NCNN debugging, DJI video replay with detection overlay, A/B model comparison, path visualisation, NFZ manual testing, and approach/descent simulation.  Not deployed to the Pi or used on flight day.
    \item[\texttt{tests/unit/}] \emph{Unit tests} (6~files, 127~test functions).  Pytest-based tests covering configuration, states, GPS transforms, planning, vision, and GPS utilities.
    \item[\texttt{tests/automated/}] \emph{Automated integration tests} (7~scripts, 33~test functions).  Extended coverage of planning, utilities, vision, geofence, and SITL smoke tests.
\end{description}

% ─────────────────────────────────────────────────────────────────────
\subsection{Flight Test Progression}\label{sec:flight-progression}

The flight scripts implement the incremental trust-building strategy.  Each script is designed so that its failure mode is benign at its tier: bench scripts cannot cause vehicle motion; passive scripts issue zero commands; waypoint scripts operate without CV decision-making.  Table~\ref{tab:flight-scripts} details the progression.
...
Table~\ref{tab:hw-tests} lists all hardware test scripts that run on the Pi bench with zero flight commands.
...
\addlinespace
\texttt{benchmark\_full.py} & Comprehensive multi-model benchmark: system info, preprocessing timing, all available TFLite/NCNN models, threading overhead, and memory profiling. & Terminal report \\
\addlinespace
\texttt{ncnn\_benchmark.py} & NCNN backend inference timing (50 runs).  Benchmarks the faster alternative backend (${\sim}72$\,ms vs 206\,ms TFLite). & Terminal report \\
\bottomrule
\end{tabular}
\end{table}
...
Calibration scripts (Table~\ref{tab:cal-tests}) measure physical parameters that the coordinate-transform pipeline (Section~\ref{sec:localisation}) depends on.  Errors in these parameters propagate directly into GPS estimation accuracy.

\begin{table}[H]
\centering
\caption{Calibration test scripts.  Each script measures a physical parameter that propagates directly into GPS estimation accuracy; errors in these values dominate the localisation error budget (Table~\ref{tab:error-budget-relative}).}
\label{tab:cal-tests}
\small
...
% ─────────────────────────────────────────────────────────────────────
\subsection{Experiment Scripts}\label{sec:experiment-scripts-app}

Six structured experiment scripts (Table~\ref{tab:experiment-scripts}) are designed to run passively during Tier~4 manual flights.  Each connects to the MAVLink telemetry stream (read-only) and the camera, but issues zero flight commands.  All output timestamped CSV files suitable for direct statistical analysis.
