...
\centering
\compactTable
\caption{Ten-phase development history with outcomes.}
...
1 & Simulation on Laptop &
    State machine (11~states, 32~transitions), lawnmower search pattern,
...
\addlinespace
10 & Safety \& Polish &
    RC override guard (LL-01).  SSSI geofence with three avoidance
    modes.  SmartEstimator greedy-cluster GPS lock.  Threaded
    inference (stream at 30\,fps, detect in background).
    127~unit tests, 23~integration tests. &
    Weeks 19--20 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{State machine} (\texttt{main.py}): 11~states from
...
The project defined three deliverable tiers to manage scope risk:

\begin{table}[H]
\centering
\compactTable
\caption{Three product levels with autonomy and demonstration scope.}
...
Flight testing follows a strict five-step ladder.  Each step isolates
one new risk factor; no step may be skipped.

\begin{table}[H]
\centering
\compactTable
\caption{Progressive flight testing ladder with scripts and risk isolation.}
...
\centering
\compactTable
\caption{Simulation vs.\ real hardware readiness (0--10 scale).}
