...
% Appendix AC (excluded from 15-page limit)
%
% STATUS: DEFINITIVE for the formal mathematical problem statement (decision vector,
%         objective functions, constraints, coupling analysis, solution approach).
%         Shares the 5-objective framework with path_optimization_definitive.tex and
%         optimization_master.tex, but presents the rigorous formulation.
...
\begin{equation}\label{eq:target-px-app}
  p_{\mathrm{px}}(h) = \frac{w_t}{\mathrm{GSD}(h)} \cdot \frac{640}{W_{\mathrm{px}}} = \frac{w_t \cdot f \cdot 640}{w_s \cdot h}
\end{equation}
...
The constraint set (Table~\ref{tab:constraints}) is derived from regulatory requirements (R01--R04), hardware limits, and operational policy:
