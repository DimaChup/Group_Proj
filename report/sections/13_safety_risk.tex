...
The survey area is adjacent to a Site of Special Scientific Interest (SSSI) designated as a no-fly zone (NFZ). Three polygon zones --- flight area (4~vertices), search area (5~vertices), and SSSI exclusion zone (7~vertices) --- are loaded from a KML file at startup and validated against hardcoded fallback coordinates in \texttt{config.py}. Because a single point of failure in boundary enforcement could result in an airspace violation, the system employs four independent protection layers (three runtime layers plus plan-time waypoint filtering):

\begin{enumerate}
    \item \textbf{Speed scalar field.} Within \texttt{NFZ\_SLOW\_ZONE\_M}\,=\,20\,m of the SSSI boundary, a \texttt{MAV\_CMD\_DO\_CHANGE\_SPEED} command linearly reduces the maximum ground speed from \texttt{NFZ\_ZONE\_MAX\_SPEED\_MPS}\,=\,3.0\,m/s at the zone edge to \texttt{NFZ\_MIN\_SPEED\_MPS}\,=\,0.3\,m/s at the boundary itself. At 0.3\,m/s the aircraft is effectively hovering; the autopilot can arrest all remaining momentum in well under one metre even in moderate wind.

    \item \textbf{Repulsive vector field.} A virtual inner polygon is offset \texttt{NFZ\_INNER\_OFFSET\_M}\,=\,20\,m inward from the SSSI boundary. When the aircraft is within \texttt{NFZ\_INNER\_RANGE\_M}\,=\,23\,m of this inner polygon --- effectively 3\,m outside the actual NFZ boundary --- a constant-magnitude repulsive velocity of \texttt{NFZ\_PUSH\_SPEED\_MPS}\,=\,3.0\,m/s is applied away from the nearest boundary segment. The constant magnitude ensures that the repulsive force always exceeds the near-zero speed cap at the boundary, preventing penetration regardless of approach velocity.

    \item \textbf{Hard boundary cutoff.} If the aircraft's GPS position falls inside the SSSI polygon (or within \texttt{NFZ\_HARD\_BOUNDARY\_M}\,=\,3\,m of it), the system immediately halts all velocity commands, saves the current state, and transitions to \textsc{Manual} mode, requiring the operator to fly the aircraft clear before autonomy can resume.
\end{enumerate}

Additionally, search waypoints within \texttt{NFZ\_WAYPOINT\_BUFFER\_M}\,=\,30\,m of the NFZ are filtered at plan time (the fourth protection layer), preventing the lawnmower pattern from routing the aircraft close to the boundary in the first place. On real hardware, a firmware-level geofence configured on the Cube autopilot provides a fifth, independent layer that triggers \texttt{LOITER} or \texttt{RTL} if all software layers fail entirely.
...
\midrule
Camera failure & Frame returns \texttt{None} & Search pattern completes without detections; aircraft returns home safely. During \textsc{Verify}, HUD displays ``CAMERA LOST''; operator should reject. \\
...
MAVLink link loss & \texttt{recv\_match} raises exception & Script issues emergency RTL; ArduCopter GCS failsafe (\texttt{FS\_GCS\_ENABLE}) provides firmware-level backup after heartbeat timeout. \\
...
The \textsc{Verify} state is the critical decision gate that prevents the system from acting on false-positive detections (see also Section~\ref{sec:operator-loop} for the full classification taxonomy). When the detection pipeline identifies a candidate target, the aircraft navigates to its estimated GPS position and holds altitude while presenting the live camera feed to the operator via the browser-based ground station. The operator classifies the detection using one of three inputs:
...
\begin{enumerate}
    \item \textbf{RC kill switch.} The RC transmitter mode channel can switch the autopilot to \texttt{STABILIZE} or \texttt{LOITER} at any time, immediately returning full manual control to the safety pilot. This hardware-level override operates independently of the onboard computer and cannot be blocked by software faults.
    \item \textbf{Keyboard abort (Ctrl+C\,/\,ESC).} Pressing either key in the Raspberry Pi terminal triggers \texttt{\_emergency\_rtl()}, which commands the Cube to enter RTL mode before exiting the script. The Cube then executes the return-to-launch sequence autonomously.
    \item \textbf{Manual override (M key).} Available from any active state, the M key transitions the system to \textsc{Manual} mode, saving the departure point for later resumption. Upon exiting manual mode, the system routes through \textsc{Return\_From\_Manual} to navigate back to the departure point before resuming the previous state.
\end{enumerate}
...
SSSI boundary incursion & L & 3 & Four-layer geofence (waypoint filter, speed field, repulsion, hard cutoff); firmware geofence backup \\
