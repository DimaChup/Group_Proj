...
\toprule
\textbf{System Property} & \textbf{Interactive} & \textbf{SITL + Sim} & \textbf{SITL + Webcam} & \textbf{Video Replay} \\
\midrule
...
Motion blur and vibration effects & $\sim$ & $\sim$ & --- & $\sim$ \\
Real GPS noise (${\sim}2$--3\,m) & $\sim$ & $\sim$ & $\sim$ & \checkmark \\
...
\begin{table}[H]
\centering
\caption{Quantitative simulation validation results.  Detection rate drops from 100\% (synthetic) to 87\% (real video), quantifying a ${\sim}$13~percentage-point sim-to-real gap; GPS estimation degrades from sub-metre to 2.3\,m CEP$_{50}$ under real noise.}
...
\begin{itemize}
    ...
    \item \textbf{Motion blur at speed.} By default, the interactive simulator and SITL produce sharp synthetic frames. The \texttt{-{}-blur} and \texttt{-{}-shake} CLI flags inject physics-based motion blur and vibration jitter into the SITL camera view, but these are approximations---the blur kernel is one-dimensional along the yaw axis and does not model sensor-specific point spread functions. Real flight at 8\,m/s with a 200\,ms exposure could produce motion blur of ${\sim}1.6$\,m ground equivalent, potentially reducing detection confidence. The DJI video provides partial evidence (recorded at a different airspeed and camera system), but the IMX296 global shutter should mitigate this for the project hardware.
