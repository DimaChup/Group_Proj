...
% =====================================================================
\subsection{Two Approaches to Eliminating Geometric Error}\label{sec:two-approaches}

The preceding analysis shows that hovering eliminates motion-induced errors.  However, two fundamentally different hover strategies exist, and the distinction has profound implications for the accuracy of the final GPS estimate.

\paragraph{Approach~1: Hover in Place.}
...
\paragraph{Approach~2: Visual Centering (implemented).}
...
Table~\ref{tab:hover-vs-centering} compares the two approaches across all geometric error sources.

\begin{table}[H]
  \centering
  \caption{Geometric error elimination: hovering in place versus visual centering.  ``Not needed'' indicates that the error source is multiplied by the zero pixel offset and does not contribute to the final estimate.}
  \label{tab:hover-vs-centering}
  \begin{tabularx}{\linewidth}{Xcc}
    \toprule
    \textbf{Error Source} & \textbf{Hover in Place} & \textbf{Visual Centering} \\
    \midrule
    Roll/pitch projection          & Eliminated       & Eliminated \\
    Motion blur                    & Eliminated       & Eliminated \\
    GSD / focal length calibration & Still required   & Not needed (zero offset) \\
    Yaw rotation accuracy          & Still required   & Not needed (zero offset) \\
    Lens distortion                & Present at edges & Minimum at optical centre \\
    GPS timing lag                 & Eliminated       & Eliminated \\
    GPS averaging                  & Single fix       & 10\,s averaged ($\sqrt{N}$ reduction) \\
    \midrule
    \textbf{Final estimate}        & Drone GPS $+$ pixel offset & Drone GPS directly \\
    \bottomrule
  \end{tabularx}
\end{table}
