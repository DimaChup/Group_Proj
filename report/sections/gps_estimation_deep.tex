...
\paragraph{Lens undistortion.}  Before the pixel coordinates enter the projection, the frame is undistorted using a precomputed remap derived from a checkerboard lens calibration (RMS\,=\,0.399\,px, Section~\ref{sec:lens-calib}).  The cost is approximately \SI{1.5}{\milli\second} per frame---negligible against the \SI{207}{\milli\second} inference time.
...
\paragraph{FOV calibration error} scales all ground offsets proportionally.  The focal length was calibrated at \SI{1}{\metre} distance (\SI{92}{\centi\metre} visible width), cross-validated at multiple altitudes from DJI video (Section~\ref{sec:fov-calib}), and found to be accurate to $\pm3$\%.
