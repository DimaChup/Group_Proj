...
A critical design decision in the landing pipeline is whether to visually servo the drone above the target before commanding a landing offset, or to proceed directly from a GPS-estimated position.  The state machine includes dedicated \textsc{Centering} and \textsc{Descending} states that enable the drone to visually servo above a detected target, progressively reducing altitude while keeping the casualty centred in the camera frame (Section~\ref{sec:state-machine}). Despite this capability, the flight-tested configuration bypasses centering entirely and proceeds directly from a confirmed detection to a \SI{7.5}{\metre} offset landing. This section justifies that design choice analytically and empirically.
