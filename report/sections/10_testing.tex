...
Of the 20 scenarios, 11 passed on the first attempt with the correct automated response already in place. The remaining 9 revealed deficiencies---missing timeouts, insufficient input validation, hardcoded parameters, or unclear error messages---that were fixed and re-verified before outdoor testing. No scenario was left unhandled: every failure mode either triggers an automated recovery (RTL, mode change, queue management) or produces an actionable operator warning with a clear next step.
