...
The mission software separates concerns across seven Python modules, each with a single responsibility:
...
The system defines 20 discrete states, grouped into four functional phases: startup, search, engagement, and recovery.
...
\begin{itemize}
    ...
    \item \textbf{HOVER} --- Fallback loiter state entered if no search waypoints are available after takeoff, with a 60-second timeout leading to \textsc{Done}.
...
A 120-second timeout ensures the drone does not hover indefinitely if the operator is unreachable; the target is automatically rejected and the search resumes. Escalating warnings are printed to the terminal at 60, 90, and 100 seconds, with a live countdown displayed on the HUD. This three-tier warning structure gives the operator progressively urgent cues:
...
\begin{enumerate}
    \item \textbf{Plan-time filtering} --- the lawnmower planner's \texttt{filter\_waypoints()} method removes any waypoint that falls within a configurable buffer of the NFZ, preventing the search pattern from ever entering the restricted area.
    \item \textbf{Runtime speed capping} --- at every loop iteration, the drone's distance to the NFZ boundary is computed using \texttt{cv2.pointPolygonTest} for signed-distance evaluation. Within the configurable slow zone (\texttt{NFZ\_SLOW\_ZONE\_M}), speed is linearly reduced from the zone maximum down to a minimum (\texttt{NFZ\_MIN\_SPEED\_MPS}), proportional to proximity. If the drone crosses the hard boundary (enters the NFZ polygon), the FSM forces an immediate transition to \textsc{Manual} and sends a zero-velocity halt command.
    \item \textbf{Repulsive potential field} --- within a configurable inner range of the boundary, a repulsive velocity vector is computed from the gradient of an inverse-distance potential field and applied as a MAVLink velocity command. This gently pushes the drone away from the boundary edge even when the current waypoint lies close to it. The repulsive vector is also rendered on the HUD as a visual indicator for the operator.
\end{enumerate}
...
The \textsc{Landing} state employs three independent touchdown detection mechanisms to handle the well-known barometric drift problem in small UAS~\cite{ardupilot_failsafe}:
...
\begin{enumerate}
    ...
    \item \textbf{Absolute timeout} --- a 90-second wall-clock timeout triggers a forced disarm (using the MAVLink emergency disarm magic number 21196), ensuring the drone does not hover indefinitely if barometric drift keeps the reported altitude above the threshold.
\end{enumerate}

\noindent If the land command does not produce descent within 5\,s, it is retried up to 5 times before forced disarm. This layered approach ensures landing completes under all observed failure modes, including barometric drift deadlock documented during bench testing.

\paragraph{State Timeouts.}
Every blocking state carries a wall-clock timeout: \textsc{Arming} warns at 120\,s, \textsc{Takeoff} at 60\,s, \textsc{Centering} aborts to \textsc{Search} at 60\,s, \textsc{Verify} auto-rejects at 120\,s, and \textsc{Hover} transitions to \textsc{Done} at 60\,s. No state can hang the mission. The centralised \texttt{\_set\_state} method resets all timeout flags and records the entry timestamp, so each state's timeout logic is self-contained.
