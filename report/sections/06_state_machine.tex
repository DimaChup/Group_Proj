...
\begin{itemize}
    \item \textbf{CENTERING} --- Navigates to the estimated GPS position of the detected target. A lock radius (\texttt{DETECT\_LOCK\_RADIUS\_M}\,=\,5\,m) prevents the drone from diverting to secondary detections that appear during approach; new detections outside this radius are queued for later investigation. A 60\,s timeout returns the drone to \textsc{Search} if convergence fails. Transitions to \textsc{Verify} when within 1\,m of the target.
    \item \textbf{DESCENDING} --- Reserved for future altitude-staged verification. Currently passes through directly to \textsc{Verify}.
    \item \textbf{VERIFY} --- The operator-in-the-loop decision point (Section~\ref{subsec:operator}). The drone holds position above the target while the operator classifies it as confirmed (\texttt{Y}), rejected (\texttt{N}), or item of interest (\texttt{I}). A 120\,s timeout auto-rejects the target to prevent indefinite hovering.
...
\begin{itemize}
    ...
    \item \textbf{RETURN\_HOME} --- Navigates to the recorded home (take-off) position. A safety guard verifies that a GPS fix was successfully obtained during arming; if not, the drone lands in place rather than flying to a potentially erroneous location.
