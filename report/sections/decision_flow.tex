% decision_flow.tex — Appendix: executive narrative of how search parameters were selected.
% Moved from the main body to reduce report length; referenced from Section \ref{sec:rationale}.

\section{Decision Flow: How We Chose Our Search Parameters}\label{app:decision-flow}

This appendix presents the full reasoning chain behind the search-parameter selection summarised in Section~\ref{sec:rationale}. It is written as a self-contained narrative so that it can be read independently; all figures and tables referenced here appear either in this appendix or in the main body, with explicit cross-references in both directions. Runtime decision-making (detect/reject, verify, manual override, geofence trigger) is covered separately in the mission flow (Section~\ref{sec:mission-flow}); this appendix focuses on the \emph{design-time} choices that determine what the mission flow operates with.
...
We adopted a clear priority stack: \textbf{safety $>$ detection $>$ speed $>$ energy}. An NFZ violation grounds the project. A missed casualty defeats its purpose. A slow search wastes the golden hour. High energy use is undesirable but survivable. This ordering underpins every trade-off described below and is consistent with the design rationale in Section~\ref{sec:rationale}. Figure~\ref{fig:decision-flow} summarises the nine-step chain.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.78\linewidth]{decision_flow}
  \caption{Nine-step decision flow for search parameter selection. Each step constrains the next, flowing from the two primary objectives (safety, detection) through secondary trade-offs (time, energy) to the selected configuration. Green steps are safety-driven; blue steps are detection-driven; grey steps are secondary optimisations.}
...
Five parameters define the search pattern (Table~\ref{tab:df-variables}; see also the parameter-chain traceability in Table~\ref{tab:param-chain} of the main body). Each affects multiple objectives; altitude alone touches all five.
...
We did not rely on intuition. A physics-based simulation evaluated \textbf{216~configurations} (6~altitudes $\times$ 36~scan angles) on the real survey polygon, modelling energy consumption, detection probability, and NFZ penalties for every combination. Table~\ref{tab:df-top3} shows the top three results.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}c c c c c c c X@{}}
    ...
    \midrule
    \textbf{1} & \textbf{35\,m} & \textbf{70\textdegree} & \textbf{8\,m/s} & \textbf{99.97\%\textsuperscript{$\dagger$}} & \textbf{12.6\,Wh} & \textbf{0.87} & \textbf{Selected --- best compromise} \\
...
\noindent\textsuperscript{$\dagger$}Assumes frame independence. Accounting for 95\% inter-frame overlap, the effective independent observations reduce to $N_{\text{eff}} \approx 1$, giving a per-pass probability of ${\sim}95\%$ (99.75\% over two passes). See Section~\ref{app:req-detail} for the full correlation-adjusted derivation.

\paragraph{A note on correlation.}
The 99.97\% figure assumes statistical independence between consecutive frames. In practice, adjacent frames share $\sim$95\% of their content (at 8\,m/s and 4.8\,FPS the drone moves only 1.67\,m between frames). This inter-frame correlation reduces the effective number of independent observations from 14 to approximately $N_{\text{eff}} \approx 1$--2, giving a single-pass detection probability closer to 95\%. However, the lawnmower pattern with 20\% overlap means that the target is scanned on at least two adjacent passes from different along-track positions, yielding a cumulative probability of $1 - (1 - 0.95)^2 = 99.75\%$. The full correlation-adjusted derivation appears in Section~\ref{app:req-detail}.
...
The final operating configuration is summarised in Table~\ref{tab:df-final}.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l c l@{}}
    ...
    \midrule
    Detection probability & $>$99.97\%\textsuperscript{$\dagger$} & 14 frames $\times$ 95\% per-frame, assuming independence (at 4.8\,FPS) \\
...
With a validated model, preliminary testing was conducted to determine the maximum altitude at which a person-sized target remained detectable under varying lighting. Preliminary simulation-based measurements and synthetic-image testing suggested the envelope shown in Table~\ref{tab:df-detection-envelope}. These figures represent conservative design targets derived from the model's performance on representative imagery; full outdoor validation across all lighting conditions remains a flight-day activity (see Section~\ref{sec:mf-buildup} for the progressive test strategy).
...
% =====================================================================
\subsection{Robustness to Runtime Edge Cases}\label{sec:df-robustness}

The parameters above were selected for nominal conditions, but the design must also tolerate off-nominal scenarios. Table~\ref{tab:df-robustness} summarises how the chosen configuration responds to the most likely runtime perturbations.

\begin{table}[H]
  \centering\compactTable
  \caption{Runtime edge cases and how the selected parameters absorb them.}\label{tab:df-robustness}
  \begin{tabularx}{\linewidth}{@{}l X@{}}
    \toprule
    \textbf{Scenario} & \textbf{Design margin} \\
    \midrule
    Detection at polygon boundary &
      The 30\,m NFZ buffer and 20\% lane overlap mean the outermost scan line sits $\geq$16\,m inside the boundary. A target detected at the edge of the camera footprint still has a GPS estimate inside the search polygon, so it passes the boundary validity check (Section~\ref{sec:mf-detection}). \\
    Multiple targets in the same pass &
      Detections are queued (up to 20) and investigated in priority order (Section~\ref{sec:mf-investigation}). The 8\,m/s speed and 14-frame dwell time ensure each target receives sufficient frames for GPS averaging before the drone moves on. \\
    GPS degradation ($>$5\,m error) &
      The 7.5\,m landing offset provides 2.5\,m of margin beyond a 5\,m GPS error. The 30\,m NFZ buffer absorbs GPS wander without risk of incursion. If GPS fix drops below 3D, the preflight gate prevents arming (Section~\ref{sec:mf-preflight}). \\
    Sustained headwind (5\,m/s) &
      Ground speed drops to 3\,m/s, increasing dwell time from 14 to 38 frames---improving detection probability. Energy consumption increases by $\sim$40\%, but the 14-minute post-search reserve (Table~\ref{tab:df-final}) absorbs this with margin. \\
    Inference rate drop (e.g.\ thermal throttle) &
      At half the nominal rate (2.4\,FPS), the target still receives 7~frames per pass---above the 5-frame empirical minimum. The conservative 8\,m/s speed provides this headroom by design. \\
    \bottomrule
  \end{tabularx}
\end{table}

\noindent These margins are not coincidental; they result from the 15--30\% safety factors applied at each step of the sequential measurement chain (Section~\ref{sec:df-sequential}). The compound effect of individual margins is that the system degrades gracefully rather than failing abruptly when any single assumption is violated.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{PLB redirect mid-search.} When a Personal Locator Beacon narrows the search area (R06), the system regenerates the lawnmower pattern over the focus polygon at reduced speed (5\,m/s) rather than abandoning the broader search. This is a procedural decision---the operator triggers it manually, and the timing depends on when the beacon data arrives, not on an optimisation variable.
\end{itemize}

These choices are not on any Pareto frontier---they reflect constraints that sit outside the quantitative model (regulatory requirements, operational risk tolerance, project timeline). Their rationale is discussed further in Section~\ref{sec:rationale}.

% =====================================================================
\subsection{Summary}\label{sec:df-summary}

The decision flow follows a strict sequential chain---vision model, detection envelope, speed, path geometry, safety margins---where each variable is locked before the next is addressed. This ordering prevents circular dependencies and ensures every parameter traces to a physical measurement or regulatory constraint. The selected configuration (35\,m, 8\,m/s, 70\textdegree, 20\% overlap, 30\,m NFZ buffer) sits at the knee of the Pareto frontier with compound safety margins that absorb GPS degradation, wind, thermal throttling, and boundary effects without requiring mid-mission re-planning. The runtime decision-making that operates within these parameters---when to investigate a detection, how to classify a target, when to override autonomy---is described in the mission flow (Section~\ref{sec:mission-flow}).
