...
\begin{table}[htbp]
\centering
\caption{Data augmentation techniques ranked by impact for aerial SAR detection.  Motion blur and brightness jitter address the two dominant real-world failure modes; lower-ranked augmentations provide diminishing returns.}
...
\begin{table}[htbp]
\centering
\caption{Recommended dataset composition for single-class aerial detection.  Real labelled frames contribute the most per image; negatives and hard negatives are essential for suppressing false positives in cluttered terrain.}
