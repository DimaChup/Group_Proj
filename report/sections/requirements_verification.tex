...
R08 & Autonomy justified & L2 semi-autonomy, operator verify gate & Design rationale, Sheridan framework & Verified (design) \\
...
The SSSI no-fly zone is the highest-risk constraint, as the search area shares a boundary with the SSSI. Four independent protection layers are implemented, illustrated in Figure~\ref{fig:geofence-diagram}:
...
\paragraph{Coverage guarantee.} The lawnmower pattern generator (\texttt{planning.py}) spaces parallel passes by the camera ground footprint width at search altitude, computed from the calibrated HFOV ($49.4^{\circ}$) and \texttt{TARGET\_ALT}~=~35\,m. This yields a ground footprint of approximately 27.6\,m per pass. The rotated-mask rasterisation ensures complete coverage of the 5-vertex search polygon with no gaps, verified by overlaying the waypoint pattern on the KML satellite image (Figure~\ref{fig:search-pattern}).
...
The system operates at Level~2 semi-autonomy: it searches and detects autonomously but requires explicit operator confirmation before committing to a target at the \textsc{Verify} state. The operator views the detection through a live MJPEG stream and classifies it as confirmed (\texttt{Y}), item of interest (\texttt{I}), or false positive (\texttt{X}). A timeout at the verify state automatically resumes the search if the operator does not respond, preventing indefinite hover. This design follows Sheridan's levels of automation~\cite{sheridan2002humans} and the SAR operational requirement that a human must validate casualty identification before deploying resources (Section~\ref{sec:rationale}).
