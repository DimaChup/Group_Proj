...
\begin{table}[htbp]
  \centering\small
  \caption{Minimum viable deliverable: components and requirement coverage.  This baseline was achieved; it satisfies R01--R10 through bench and simulation testing without requiring outdoor flight.}\label{tab:mvd}
...
\begin{table}[htbp]
  \centering\small
  \caption{Target deliverable: full autonomous mission.  Each capability adds one layer of outdoor validation; weather cancellation prevented progression beyond Level~1.}\label{tab:target}
  \begin{tabularx}{\linewidth}{@{}p{4.2cm}X c@{}}
    \toprule
    ...
    Onboard AI detection & YOLOv8n TFLite inference at 4.8\,FPS triggers automatic transition to \textsc{Centering} state. Detection queue with spatial clustering filters duplicates. & R05 \\
...
\begin{itemize}
  ...
  \item \textbf{NCNN inference backend.} The NCNN export of the retrained YOLOv8n model is available in \texttt{cv\_models/\allowbreak{}sar\_v2\_1088/\allowbreak{}ncnn/} and is expected to achieve ${\sim}$15\,FPS on the Pi~5 CPU, tripling the current TFLite throughput of 4.8\,FPS and enabling detection during faster transit legs.
...
\begin{table}[htbp]
  \centering\small
  \caption{Tier structure and verification gates.  Each tier defines a pass/fail gate; failure triggers a graceful fallback to the previous tier rather than mission abort.}\label{tab:tier-evolution-summary}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}p{2.6cm}p{3.0cm}X@{}}
    ...
    CV fails in field (no detections) & Zero detections after full search pass & Swap to COCO person detector (\texttt{models/human.tflite}, 80-class YOLOv8n, \SI{13}{\mega\byte}). If still no detections, reduce confidence threshold from 0.2 to 0.1 via \texttt{config.py} and repeat. \\
...
\begin{table}[htbp]
  \centering\small
  \caption{Contingency-to-test-script mapping.  Each failure scenario can be verified on-site before flight using the listed script; no scenario relies solely on in-flight discovery.}\label{tab:test-scripts}
  \begin{tabularx}{\linewidth}{@{}p{2.2cm}p{3.2cm}X@{}}
    \toprule
    \textbf{Scenario} & \textbf{Script} & \textbf{What it proves} \\
    \midrule
    GPS lock failure & \texttt{gps\_health.py} & Step-by-step GPS verification: satellite count, fix type, HDOP. Confirms GPS is usable before takeoff. \\
    CV fails in field & \texttt{benchmark.py} & Runs 50 inference cycles on a static image; reports detection rate and confidence. \\
    Pi crash recovery & \texttt{preflight.py} & Full connectivity check: camera, Cube link, model load, GPS. Run after restart to confirm all subsystems online. \\
    Altitude too low & \texttt{1\_passive\_ flight.py} & Passive detection during manual RC flight at multiple altitudes. Review log for minimum detection altitude. \\
    Wind assessment & \texttt{cube\_monitor.py} & Live telemetry: wind estimate, ground speed, attitude. Pilot assesses conditions before committing. \\
    Model swap & \texttt{detection\_ snapshot\_test.py} & Single-frame detection on a static image with any \texttt{.tflite} model. Confirms replacement model works. \\
    Full fallback & Mission Planner AUTO + \texttt{passive\_watch.py} & Pre-uploaded waypoints in AUTO; Pi runs passive CV with zero commands. No custom flight code needed. \\
    \bottomrule
  \end{tabularx}
\end{table}
