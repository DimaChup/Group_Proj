...
\textbf{states.py} (22\,lines) defines a state enumeration with 19 named states spanning the full mission lifecycle, including \texttt{INIT}, \texttt{CONNECTING}, \texttt{ARMING}, \texttt{TAKEOFF}, \texttt{TRANSIT\_TO\_SEARCH}, \texttt{SEARCH}, \texttt{CENTERING}, \texttt{DESCENDING}, \texttt{VERIFY}, \texttt{APPROACH}, \texttt{LANDING}, and \texttt{DONE}. Each state maps to a handler function in the orchestrator. Additional states cover manual override (\texttt{MANUAL}), return-to-home (\texttt{RETURN\_HOME}), and hover behaviours.
...
\begin{enumerate}
    ...
    \item \textbf{Detection.} The YOLOv8n model runs inference and returns pixel coordinates $(x, y)$ in model space along with a confidence score. On the Pi, this takes approximately 200\,ms per frame using the TFLite XNNPACK CPU delegate~\cite{david2021tensorflow, tflite}.
...
Due to a known incompatibility between Python~3.13 and the pyserial library (dropped bytes causing \texttt{BAD\_DATA} errors), the system interposes a MAVProxy bridge process~\cite{ardupilot_mavproxy}. MAVProxy reads the serial port reliably using its own C-level serial handler and republishes the MAVLink stream over UDP:
